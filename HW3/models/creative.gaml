...
//				write act_util_list;
	...
	reflex dancing when: status=2 {
		do wander;
		if(flip(0.001)){
			do start_conversation with: [to :: list(fav_stage), protocol :: 'fipa-contract-net', performative :: 'inform', contents :: ['Emergency'] ];
			write "Dead: " + self;
			do die;
		}
	}
...
//		}
	...
	reflex receiveEmergengy when: !(empty(informs)) {
		message msg <- informs[0];
		
		if(msg.contents[0] = "Emergency"){
			write "Announcing emergency";
			do start_conversation with: [to ::list(Guests), protocol :: 'fipa-contract-net', performative :: 'inform', contents :: ['end'] ]; 
			whenToStart <-0;
			end <-true;
			ongoing<-false;
